  ...
  return new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        let finalHeadings: EnhancedHeading[];
        if (useCorrected && corrected) {
          // Re-run programmatic instruction builder on corrected headings so
          // structureInstructions match the (possibly changed) heading text.
          const rerunCtx: InstructionContext = {
            topic: params.topic,
          ...
          const rerunProgrammatic = assignPatternsAndInstructions(
            corrected.map((h) => ({ level: h.level, text: h.text })),
            rerunCtx
          );
          console.log(`[Step 11] Re-ran programmatic builder on ${rerunProgrammatic.length} corrected headings`);

          // Merge: take LLM corrections for level/text/intent, but use fresh
          // programmatic data for structure instructions and all pattern fields.
          finalHeadings = corrected.map((h, i) => {
            const prog = rerunProgrammatic[i];
            if (!prog) return h;
            return {
              ...h,
              structureInstructions: prog.structureInstructions,
              structurePattern: prog.structurePattern,
              wordCountTarget: prog.wordCountTarget,
              contentDesignPattern: prog.contentDesignPattern,
              snippetTarget: prog.snippetTarget,
              paaTarget: prog.paaTarget,
            };
          });
        } else {
...
Return JSON:
{
  ...
  "correctedHeadings": [{ "level": number, "text": string, "intent": string }]
}

IMPORTANT: Only include "correctedHeadings" if score < 8. If score >= 8, omit correctedHeadings entirely.
When providing correctedHeadings, ONLY modify "level", "text", and "intent". Do NOT include structureInstructions, targetQueries, serpFeatures, ruleCodes, or wordCountTarget — those are managed programmatically and will be regenerated automatically.`,
      },
      {
        role: "user",
...
${headingList}`,
  ...
  // IMPORTANT: Never overwrite structureInstructions — those are programmatically
  // generated by assignPatternsAndInstructions() in Step 9 and must be preserved.
  // The LLM correction step only adjusts level, text, and intent.
  ...
  if (parsed.correctedHeadings && parsed.score < 8) {
    correctedHeadings = parsed.correctedHeadings.map((corrected, i) => {
      ...
      return {
        ...
        // structureInstructions, structurePattern, wordCountTarget,
        // contentDesignPattern, snippetTarget, paaTarget are ALL preserved
        // from the programmatic builder — never overwritten by LLM.
      };
    });
  }
